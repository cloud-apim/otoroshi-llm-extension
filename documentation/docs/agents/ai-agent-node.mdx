...
| `search_engines` | array | no | List of [Search Engine](/docs/search-engines/introduction) ids the agent can use as a web `search` tool |
...
Reference [Search Engines](/docs/search-engines/introduction) to give the agent a web `search` tool:
...
```json
{
  ...
  "provider": "provider_xxxxx",
  "description": "An assistant with web search access",
  "instructions": ["Answer using up-to-date web information and cite your sources."],
  "input": "${input.question}",
  "search_engines": ["search-engine_xxxxx"],
  "result": "agent_response"
}
```
...
The `content_to_markdown` tool uses [Kreuzberg](https://github.com/kreuzberg-dev/kreuzberg) to convert documents (PDF, DOCX, HTML, images, etc.) into markdown text. The agent can provide either a URL or base64-encoded content.
...
- `redis://` or `rediss://` — uses a Redis HASH per namespace (`HSET`/`HGET`/`HDEL`/`HGETALL`)
- `postgresql://` — uses a PostgreSQL table `otoroshi_agent_kv` (created automatically)

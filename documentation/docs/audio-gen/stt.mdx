...
| **OVH AI Endpoints** 🇫🇷 🇪🇺 | (your transcription model) |
...
A generic provider for any transcription service that speaks the OpenAI `/audio/transcriptions` format. Point it at your endpoint with `base_url` and `token`, and give it a display name with `provider_name`. It uses the same STT parameters as OpenAI.
...
OVH AI Endpoints 🇫🇷 🇪🇺 provides transcription only (no TTS, no translation) through its unified OpenAI-compatible API. It uses the same STT parameters as OpenAI — just set your `token` and the transcription `model`.
...
## Full entity example with OVH AI Endpoints

```js
{
  "id": "audio-gen-model_xxxxxxxxx",
  "name": "OVH Transcription",
  "description": "OVH AI Endpoints speech-to-text",
  "provider": "ovh-ai-endpoints",
  "config": {
    "connection": {
      "base_url": "https://oai.endpoints.kepler.ai.cloud.ovh.net/v1",
      "token": "${vault://local/OVH_API_KEY}",
      "timeout": 180000
    },
    "options": {
      "stt": {

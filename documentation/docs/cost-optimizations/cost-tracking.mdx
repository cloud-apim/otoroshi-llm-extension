...
The cost tracking uses a **three-layer pricing strategy**:

1. **Built-in LiteLLM prices**: the full [LitLLM](https://docs.litellm.ai/) model price dictionary, bundled with the extension
2. **Built-in custom prices**: additional prices defined by the extension for models not in LiteLLM
3. **User-defined prices**: your own prices via the `custom-prices` config (highest priority, overrides the others)

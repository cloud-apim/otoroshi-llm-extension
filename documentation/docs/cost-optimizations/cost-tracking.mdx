...
| `openrouter-catalog.enabled` | boolean | `true` | Keep OpenRouter model prices in sync with its public catalog |
...
2. **The model price table**, used whenever the provider reports nothing, resolved in this order:
   ...
   - **The OpenRouter catalog**, kept in sync with the prices OpenRouter publishes, so new models are priced the day they ship
...
Cost tracking natively supports the following providers:

...
| `embedding_model` | string | default model of the embedding model | Model of that Embedding Model entity to use |
...
`embedding_model` is optional: without it, the default model of the Embedding Model entity is used. The cached
vectors are only comparable with vectors of the same model, so changing the model starts a fresh cache.
...
- Embedding HASH keys also expire via `PEXPIRE`, automatically removing them from the search index
- The RediSearch index is created automatically on first use with the correct vector dimensions

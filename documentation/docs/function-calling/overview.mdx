...
## Picking the tools of a call

The tools of a provider are offered on every call it serves. A consumer that only needs some of them names
them in `allowed_tools`, with the ids of the tool entities:

```js
{
  "model": "gpt-4o-mini",
  "messages": [{ "role": "user", "content": "what is the weather in Paris?" }],
  "allowed_tools": ["tool-function_xxx"]
}
```

| Request | Tools offered to the model |
|---------|----------------------------|
| no `allowed_tools` | every tool attached to the provider |
| `["tool-function_xxx"]` | that one, if the provider carries it |
| `[]` | none |

A call can only **narrow** what its provider carries: an id the provider does not have is ignored, so a
consumer never reaches a tool you did not attach. The field covers tool functions, MCP connectors, A2A
connectors and search engines alike, and it stays in the gateway — the provider API never sees it.

It works on the chat completions, completions and responses endpoints, and the AI Studio chat uses it: the
**Tools** list of its request settings picks what the models may call in a conversation.
...
| `outputSchema` | object \| null | Optional MCP output schema, forwarded as `outputSchema` when this tool is exposed through an [MCP server plugin](/docs/mcp/mcp-plugins) |
| `annotations` | object \| null | Optional MCP tool annotations (e.g. `readOnlyHint`, `destructiveHint`, `idempotentHint`, `openWorldHint`, `title`), forwarded as `annotations` |
| `meta` | object \| null | Optional free-form metadata, forwarded as `_meta` (used by MCP apps and hybrid-auth scenarios) |
| `backend` | object | Backend configuration (kind + options) |

:::note MCP metadata fields
`outputSchema`, `annotations` and `meta` are **optional** and only relevant when the tool function is exposed through an [MCP server plugin](/docs/mcp/mcp-plugins). They have no effect on the standard LLM tool-calling loop. When unset they are omitted from the `tools/list` output (no `null` value is emitted).
:::

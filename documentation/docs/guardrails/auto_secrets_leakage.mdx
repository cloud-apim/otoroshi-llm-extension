...
|-----------|------|:--------:|---------|-------------|
| `provider` | string | Yes | — | Reference ID of the LLM provider used to evaluate messages for secret leakage. Must be different from the main provider, unless a `model` is set. |
| `model` | string | No | default model of the provider | Model of the validation provider used to judge the messages, for instance a smaller and faster one. With a model set, the guarded provider can judge its own messages with another of its models. |

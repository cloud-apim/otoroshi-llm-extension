...
We provide an [Open Responses API compatible proxy](/docs/llm-gateway/open-response-proxy) that allows any client speaking the [Open Responses](https://www.openresponses.org/) specification to use any LLM provider managed by Otoroshi. Open Responses is an open-source specification for multi-provider, interoperable LLM interfaces with built-in support for agentic workflows, tool calling, and streaming with semantic events. See the [dedicated documentation page](/docs/llm-gateway/open-response-proxy) for details.

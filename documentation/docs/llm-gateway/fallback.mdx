...
| `provider_fallback_model` | string | default model of the fallback provider | Model of the fallback provider to use. When set, the model asked for the primary provider is not sent to the fallback, so a fallback on another vendor always gets a model it serves. |
...
A request for a model the primary provider does not allow, through its own [model restrictions](/docs/llm-gateway/setup-provider#model-restrictions) or those of the API key or user, is refused right away: it is not retried on the fallback, and it does not count as a failure of the primary provider. The fallback only serves the requests the primary provider would have accepted.

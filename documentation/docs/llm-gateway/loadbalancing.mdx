...
The `refs` array can contain either simple provider IDs (strings) or objects with weight, selector and model:
...
**With a model** — spread the traffic over several models, even of the same provider:

```js
{
  "refs": [
    { "ref": "provider_openai_1", "weight": 3, "model": "gpt-4o-mini" },
    { "ref": "provider_openai_1", "weight": 1, "model": "gpt-4o" }
  ]
}
```

A target without `model` uses the default model of its provider. A target with a `model` always uses it, even
when the request asks for another model.

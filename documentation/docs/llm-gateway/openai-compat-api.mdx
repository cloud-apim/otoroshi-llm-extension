...
| `pricing` | The prices cost tracking bills the model with, in dollars per token, as strings, in the OpenRouter format: `prompt`, `completion`, `input_cache_read`, `input_cache_write`, `internal_reasoning`, `audio`, `audio_output`, `image`, `image_output` |

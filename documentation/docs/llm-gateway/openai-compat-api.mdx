...
| `pricing` | Prices in dollars per token, as strings, in the OpenRouter format: `prompt`, `completion`, `input_cache_read`, `input_cache_write`, `internal_reasoning`, `audio`, `audio_output`, `image`, `image_output` |
...
The prices are exactly the ones your budgets are charged with: `sources.pricing.source` is `price-table` when they come from the price table (including your own `custom-prices`), and `models.dev` when the catalog fills the gap.

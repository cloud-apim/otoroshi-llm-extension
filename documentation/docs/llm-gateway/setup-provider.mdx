...
| `responses` | boolean | `false` | Call the provider's native [Responses API](/docs/llm-gateway/openai-responses-proxy#native-responses-api) endpoint for `/responses` requests instead of translating them to chat completions. Available on OpenAI, OpenAI-compatible providers, Azure OpenAI, Groq, x.ai and Ollama |
...
| `search_engines` | array | `[]` | List of [Search Engine](/docs/search-engines/introduction) IDs, exposed to the model as a web `search` tool |
...
### Target a model of one provider

Behind the [unified API](/docs/llm-gateway/openai-compat-api), the same model can be called under several names: `model`, `<provider>/<model>` or `<provider>###<model>`, the provider being its name (as a slug) or its id. A pattern can use any of these forms, so a rule can allow or block a model on one provider only:

```js
{
  "models": {
    "include": ["openai/gpt-4o", "mistral-.*"],
    "exclude": ["azure###.*"]
  }
}
```

Here `gpt-4o` is allowed only through the `openai` provider, every `mistral-*` model is allowed wherever it is served, and nothing from the `azure` provider goes through. A pattern without a provider, like `mistral-.*`, applies to every provider.
...
Patterns accept the same `model`, `<provider>/<model>` and `<provider>###<model>` forms as the provider restrictions. Since the list is comma-separated, a pattern cannot contain a comma. A blank value restricts nothing.
...
All three levels of restrictions are combined: a model is allowed only if it passes the **provider** restrictions, **AND** the **API key** restrictions (if any), **AND** the **user** restrictions (if any). A provider that restricts nothing does not lift the restrictions of its API keys and users. If a model is denied at any level, the request is rejected with `{"error": "you can't use this model"}`, before the provider is called. A request without a model goes to the default model of the provider, which patterns can target as `<provider>/_default`.

To restrict a user to GPT-4o on OpenAI and to Mistral models anywhere:

```json
{
  "metadata": {
    "ai_models_include": "openai/gpt-4o,mistral-.*"
  }
}
```

This mechanism applies to all model types: LLM providers, embedding models, audio models, image models, video models, moderation models and OCR models.

#### Load balancers, routers and fallbacks

[Load balancers](/docs/llm-gateway/loadbalancing), [Otoroshi routers](/docs/llm-gateway/otoroshi-router) and [fallbacks](/docs/llm-gateway/fallback) hand requests over to other providers. The restrictions of an API key or a user apply to what the consumer calls: the load balancer or the router, with the model it asks for (`my_balancer/_default`, `router/auto-router`...). Where the request then goes is your routing configuration: the providers it reaches apply their own restrictions, not those of the consumer. Allowing a consumer to use a router is enough, there is no need to list its candidates.

```json
{
  "metadata": {
    "ai_models_include": "router/auto-router,my_balancer/_default"
  }
}
```

A fallback only takes over the requests the primary provider accepted: a model refused on the primary provider is never served by its fallback.

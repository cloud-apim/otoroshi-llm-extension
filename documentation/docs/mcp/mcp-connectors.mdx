...
| **Streamable HTTP (2025-11-25)** | `http` | Connect to a remote MCP server over the Streamable HTTP transport, with the `initialize` handshake. |
| **Streamable HTTP (2026-07-28)** | `http_2026_07_28` | Connect to a remote MCP server speaking the [stateless `2026-07-28` revision](#stateless-mcp-servers-2026-07-28). |
...
:::tip Full-fidelity passthrough with the Streamable HTTP transports
When this connector is re-exposed through an [MCP server plugin](/docs/mcp/mcp-plugins), the **Streamable HTTP** transports (`http` and `http_2026_07_28`) preserve the full upstream payload: tool `outputSchema`, `annotations`, `_meta`, `title`, and rich `tools/call` results (`structuredContent`, resource links, images). Other transports go through an abstraction layer that recovers `_meta`/`annotations`/`title` but not `outputSchema`. Prefer them for MCP apps that rely on this metadata.
:::

The `http` transport asks for protocol version `2025-11-25` by default. Set the connector metadata `protocol_version` to request another one.

## Stateless MCP servers (2026-07-28)

The `http_2026_07_28` transport talks to MCP servers implementing the stateless revision of the protocol, including any Otoroshi [MCP exposition](/docs/mcp/mcp-plugins#protocol-versions) configured with `protocol_version: "2026-07-28"`. It takes the same options as `http`:

```js
{
  "transport": {
    "kind": "http_2026_07_28",
    "options": {
      "url": "https://mcp.example.com/mcp",
      "headers": {
        "Authorization": "Bearer ${vault://local/mcp-token}"
      },
      "timeout": 30000
    }
  }
...
Every request is self-contained and fully compliant with the revision:

- the server is checked with `server/discover` and must support `2026-07-28`,
- each request carries the protocol version, the client identity (connector name and metadata `version`) and the client capabilities in `_meta`, along with the `MCP-Protocol-Version`, `Mcp-Method` and `Mcp-Name` headers,
- tool parameters annotated with `x-mcp-header` are mirrored into `Mcp-Param-*` headers (Base64 encoded when needed). Tools whose annotations are invalid are left out of the tool list, and if the server reports a header mismatch after a tool definition changed, the connector refreshes the definitions and retries once,
- paginated lists are fetched page by page, and servers that ask for a retry with a `requestState` are followed transparently.
...
| `mcp_connector_kind` | string | The transport kind (`stdio`, `sse`, `ws`, `http`, `http_2026_07_28`, `http_langchain`, `meta`, `openapi`) |
...
:::tip
The same meta surface can be exposed **server-side** on a virtual server via [`expose_as_meta`](/docs/mcp/mcp-virtual-servers#meta-mode-tool-virtualization), without creating a meta connector at all.
:::

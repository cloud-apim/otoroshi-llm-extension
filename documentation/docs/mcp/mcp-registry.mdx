...
| `url` | string | — | Public streamable-http URL exposed for this server (the `server.json` remote). **Set it manually for now** — deriving it from the routes that expose the server is not done yet. If empty, no remote is published (clients still discover auth via the server's own `.well-known`). |
...
- **Auto-deriving the remote `url`** by scanning the routes that expose the server (manual `url` for now).

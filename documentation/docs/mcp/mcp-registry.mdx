---
sidebar_position: 4
---

# MCP Registry

Otoroshi can publish your governed [MCP virtual servers](/docs/mcp/mcp-virtual-servers) to the standard **[MCP registry](https://github.com/modelcontextprotocol/registry)** format, so that **registry-aware MCP clients** in your organization discover the sanctioned servers automatically — and connect **through the gateway** (so OAuth and the [Zero-Trust controls](/docs/mcp/mcp-virtual-servers#zero-trust-controls) apply end to end).

It is the difference between *"a pile of connection configs"* and *"a curated, approved, browsable, versioned catalogue of sanctioned MCP capabilities"*.

:::info First version — publish-only
This first version is **publish-only**: Otoroshi *exposes* its governed catalogue. Importing servers from a remote registry, identity-aware filtering of the listing, cursor pagination, an approval *workflow*, and auto-deriving the remote `url` from the routes are **not** included yet — see [Scope & limits](#scope--limits).
:::

## How it works

A registry is essentially **a list of [`server.json`](https://github.com/modelcontextprotocol/registry/blob/main/docs/reference/server-json/generic-server-json.md) documents**, one per MCP server. In Otoroshi a registry-server maps **1:1 to a virtual server**: the `server.json` is a *projection* of the virtual server. There is no separate entity — publication is a small block of metadata on the virtual server itself.

The lifecycle is intentionally minimal: a server is either `published` (and optionally `deprecated`). Flipping `published` is your **approval gate**, gated by the usual Otoroshi role/RBAC — no extra workflow.

## The `registry` block

Configured on the [MCP Virtual Server](/docs/mcp/mcp-virtual-servers) entity (entity-level metadata, not a per-route override):

| Field | Type | Default | Description |
| --- | --- | --- | --- |
| `published` | boolean | `false` | List this server in the registry endpoints. This is the approval gate. |
| `name` | string | — | Reverse-DNS namespaced name (e.g. `io.acme/github`). Defaults to a slug of the server name (`io.cloud-apim/<slug>`). |
| `version` | string | `1.0.0` | Semantic version advertised in `server.json`. |
| `title` | string | — | Human-readable title. Defaults to the server name. |
| `url` | string | — | Public streamable-http URL exposed for this server (the `server.json` remote). The editor has a **"Detect from route" assistant** that scans the route(s) exposing this server, suggests the URL and shows the detected auth (OAuth / apikey / mTLS) — you confirm or override. If empty, no remote is published (clients still discover auth via the server's own `.well-known`). |
| `deprecated` | boolean | `false` | Marks the registry status as `deprecated` while keeping the entry listed. |

## The `server.json` projection

A published server is projected to the official `server.json` format (schema `2025-12-11`). The `server.json` itself has **no status field** — the registry layer adds it under the `_meta` envelope:

```json
{
  "$schema": "https://static.modelcontextprotocol.io/schemas/2025-12-11/server.schema.json",
  "name": "io.acme/github",
  "description": "Hardened GitHub MCP via Otoroshi",
  "title": "GitHub",
  "version": "1.0.0",
  "remotes": [{ "type": "streamable-http", "url": "https://gw.acme/mcp/github" }],
  "_meta": {
    "io.modelcontextprotocol.registry/official": { "status": "active", "isLatest": true },
    "com.cloud-apim.otoroshi/governance": { "id": "mcp-virtual-server_xxx", "tags": ["platform"] }
  }
}
```

- `name` / `version` / `title` come from the `registry` block (with the slug / entity-name fallbacks).
- `remotes` is emitted only when `url` is set; the type is always `streamable-http`.
- `status` is `active`, or `deprecated` when `registry.deprecated` is on.
- Governance metadata (entity id, tags) is added under our own reverse-DNS `_meta` key, which does not break the standard.

## One-click setup — the registry preset

The simplest way to expose the registry is the **`Cloud APIM - MCP Registry`** preset. Add it to a route and it wires both plugins at once:

| Option | Default | Wires |
| --- | --- | --- |
| `well_known_path` | `/.well-known/mcp-registry` | the discovery document |
| `api_base_path` | `/v0` | the registry API on `/v0/.*` |
| `registry_url` | — | base URL advertised by the discovery doc (defaults to this host + base path) |
| `schema_version` | `2025-12-11` | the `server.json` schema version |

So out of the box the discovery doc is served on **`/.well-known/mcp-registry`** and the API on **`/v0/servers`**. If you'd rather wire the plugins by hand, the two are described below.

## Exposing the registry — two plugins

Both are standard `NgBackendCall` plugins; put them on routes on your gateway. They are **publish-only**: only enabled + `published` servers are listed.

### `Cloud APIM - MCP Registry API`

Serves the standard registry REST API. Put it on a route matching your registry base path (e.g. `/v0/servers`):

- `GET /v0/servers` — the listing:
  ```json
  {
    "servers": [ { "name": "io.acme/github", "version": "1.0.0", "...": "..." } ],
    "metadata": { "count": 1 }
  }
  ```
- `GET /v0/servers/{name}` — a single server by its (possibly namespaced) name, e.g. `GET /v0/servers/io.acme/github`. Returns the `server.json`, or `404` if it is unknown or not published.

### `Cloud APIM - MCP Registry discovery (.well-known)`

Serves a small discovery document pointing clients at the registry API. Put it on your well-known path.

```json
{
  "registry": "https://gw.acme/v0",
  "servers_endpoint": "https://gw.acme/v0/servers",
  "schema_version": "2025-12-11",
  "server_json_schema": "https://static.modelcontextprotocol.io/schemas/2025-12-11/server.schema.json"
}
```

:::note
The exact `.well-known` path/format for an MCP *registry* is not strongly standardized yet, so the document is kept minimal and the path is set on the route. We will align it if/when the spec firms up.
:::

## End-to-end flow

> An admin approves **GitHub** (a virtual server) by setting `registry.published = true` → the registry lists it as a `server.json` (`io.acme/github`, remote `https://gw.acme/mcp/github`) → any registry-aware MCP client in the company discovers it → connects **through the gateway** → governed + Zero-Trust-protected end to end.

The discovery endpoint funnels clients onto the governed path: instead of each team hand-configuring connections, a client just needs the registry URL and auto-discovers every sanctioned server (with its auth requirements).

## Scope & limits

A note on the URL: the **"Detect from route" assistant** suggests `registry.url` (and the auth) by scanning the routes that expose the server, but the value is **stored and human-confirmed** — a gateway can't always know its own public URL behind a proxy/LB, and a server may be exposed on several routes. The projection itself never scans.

Deferred for a later iteration:

- **Fully-automatic remote `url`** resolved live at projection time (no stored value).
- **Identity-aware filtering** of the listing (a team only sees what its RBAC/scopes allow).
- **Cursor pagination** (`metadata.next_cursor`).
- An **approval workflow** (we stay on `published` + admin role).
- **Import / federation** from a remote registry, and **multi-kind** catalogue (APIs, workflows).

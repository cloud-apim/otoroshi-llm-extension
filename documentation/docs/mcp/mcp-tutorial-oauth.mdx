...
In this tutorial, you will build a complete, production-ready MCP server that exposes geocoding tools from the French national geocoding API ([Géoplateforme IGN](https://data.geopf.fr/geocodage/)). All the exposition settings live on a reusable [**MCP Virtual Server**](/docs/mcp/mcp-virtual-servers), which the route references with `server_ref`. The server is protected with OAuth 2.0 (the built-in `enforce_oauth` option) and exposes the standard `.well-known/oauth-protected-resource` discovery document via the dedicated `McpProtectedResourceMetadata` plugin.
...
```
   ...
   MCP Client ──── request ────►│                                  │
                                │  1. McpProtectedResourceMetadata │
                                │     (.well-known/oauth-pr...     │
                                │     auto-generated from OIDC)    │
                                │                                  │
                                │  2. McpRespEndpoint              │
                                │     enforce_oauth: true          │
...
The `config` block is the shared exposition config. You can later add [audience binding](/docs/mcp/mcp-virtual-servers#oauth-20-security), [scope→tool RBAC](/docs/mcp/mcp-virtual-servers#scope-based-tool-authorization), [meta mode](/docs/mcp/mcp-virtual-servers#meta-mode-tool-virtualization), or [rate limiting & caching](/docs/mcp/mcp-virtual-servers#per-tool-rate-limiting-and-result-caching) here — without touching the route.
...
- the **Protected Resource Metadata** document ([RFC 9728](https://datatracker.ietf.org/doc/html/rfc9728)) on `/.well-known/oauth-protected-resource` (customizable via `well_known_path`).

OAuth enforcement, the auth module and the tools all come from the referenced [virtual server](#step-3-create-the-mcp-virtual-server) — so the route configuration is just the preset and one `server_ref`.
...
:::tip Manual wiring
Prefer to control the plugins yourself (e.g. to add other plugins on the route)? You can add the `McpRespEndpoint` and `McpProtectedResourceMetadata` plugins manually instead of the preset — see [MCP server exposition](/docs/mcp/mcp-plugins).
:::

### What the preset wires up

The preset expands into two plugins on the route. You don't configure them directly — everything comes from the virtual server — but here is what each does.

#### Protected Resource Metadata (RFC 9728)

It serves the [OAuth Protected Resource Metadata](https://datatracker.ietf.org/doc/html/rfc9728) document at `/.well-known/oauth-protected-resource`. Instead of hardcoding the metadata as a static JSON string, it **automatically derives** the document from the OIDC auth module of the virtual server:
...
This document tells MCP clients how to authenticate — which authorization server to use, which scopes to request, and where to find the public keys for token verification.

:::tip
If you change your OIDC provider or its configuration, the PRM document updates automatically — no need to manually edit a static JSON body.
:::

#### OAuth enforcement on the MCP endpoint

OAuth enforcement (`enforce_oauth`) and the auth module come from the [virtual server](#step-3-create-the-mcp-virtual-server). When a request arrives without a valid token, the endpoint returns:
...
The [MCP Inspector](https://github.com/modelcontextprotocol/inspector) is an interactive developer tool for testing and debugging MCP servers. You can use it to connect to your OAuth-protected MCP server with a visual UI.
...
This opens the Inspector UI at [http://localhost:6274](http://localhost:6274).
...
npx @modelcontextprotocol/inspector \
  ...
  --tool-arg address="73 avenue de Paris, Saint-Mandé" \
  --tool-arg limit=1
...
- **Fine-tune access control**: Use the [MCP virtual server filters](/docs/mcp/mcp-virtual-servers#filtering) to control which tools are exposed to which clients
...
- **Custom scopes**: the advertised scopes come from the auth module's `scope` field — edit it to change which scopes the PRM document advertises
- **Custom resource identifier**: the `resource` defaults to the request's protocol and host; to override it (e.g. behind a reverse proxy or load balancer), [wire the `McpProtectedResourceMetadata` plugin manually](/docs/mcp/mcp-plugins) instead of the preset and set its `resource` field (it also accepts a `scopes_supported` override there)

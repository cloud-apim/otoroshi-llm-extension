...
The OAuth settings and the tool list are defined once on an **MCP Virtual Server**; the route's `McpRespEndpoint` plugin just references it with `server_ref`.
...
Create a route in Otoroshi that exposes the virtual server as an MCP Streamable HTTP server, protected by OAuth 2.0.

This route uses two plugins, both pointing at the virtual server with `server_ref`:

1. **McpProtectedResourceMetadata** — serves the [RFC 9728](https://datatracker.ietf.org/doc/html/rfc9728) Protected Resource Metadata document at `/.well-known/oauth-protected-resource`, automatically derived from the OIDC auth module of the referenced virtual server (authorization server URL, JWKS URI, scopes)
2. **McpRespEndpoint** — references the virtual server and handles MCP protocol requests with built-in OAuth enforcement (from the virtual server): it validates the Bearer token and returns a `401` with a `WWW-Authenticate` header pointing to the PRM document if the token is missing or invalid
...
This plugin serves the [OAuth Protected Resource Metadata](https://datatracker.ietf.org/doc/html/rfc9728) document at `/.well-known/oauth-protected-resource`. Instead of hardcoding the metadata as a static JSON string, it **automatically derives** the document from the OIDC auth module — taken from the referenced virtual server (`server_ref`), or set directly via `auth_module_ref`:
...
OAuth enforcement (`enforce_oauth`) and the auth module live on the [virtual server](#step-3-create-the-mcp-virtual-server); the plugin only needs `server_ref`.

| Field | Value | Description |
|-------|-------|-------------|
| `server_ref` | `mcp-virtual-server_geocoding` | References the virtual server, which carries the tools, `enforce_oauth`, and the auth module used to verify Bearer tokens. |

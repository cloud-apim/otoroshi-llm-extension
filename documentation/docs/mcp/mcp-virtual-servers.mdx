...
| `description_guardrails` | array | `[]` | **B. Tool-poisoning / prompt-injection scanning** of tool **descriptions** at `tools/list`. Reuses the [guardrails](/docs/guardrails) engine — each item is `{ "enabled": true, "id": "<guardrail>", "config": {...} }` (e.g. `prompt_injection`, `pif`, `secrets_leakage`, `regex`, `contains`, `wasm`). |

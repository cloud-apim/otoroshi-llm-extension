...
All the exposition settings — filtering, OAuth, scopes, meta mode, rate limiting & caching, managed resources/prompts, overlays, and [zero-trust controls](/docs/mcp/mcp-virtual-servers#zero-trust-controls) (anti-rug-pull pinning, prompt-injection scanning, PII/secrets redaction) — are bundled in a reusable [**MCP Virtual Server**](/docs/mcp/mcp-virtual-servers) that the plugins reference with `server_ref`.

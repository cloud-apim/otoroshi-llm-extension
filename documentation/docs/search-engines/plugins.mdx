...
For a [`rag`](./providers#rag-knowledge-base) engine, `provider` is `rag`, each result's `title` is the matched document id, `url` is empty, `snippet` holds the retrieved passage, and `score` is the vector similarity.

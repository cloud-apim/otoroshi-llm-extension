...
object OtoroshiAssistant {
  ...
  private val basePrompt: String =
    s"""You are the Otoroshi Assistant, an expert AI helper embedded in the Otoroshi admin UI.
       ...
       |- Search and reference Otoroshi documentation when relevant. The official manual is at https://www.otoroshi.io/docs/. When the user's question maps to a specific section, point them to the most relevant page. You can find more about the otoroshi ecosystem at https://www.otoroshi.io/ecosystem. You can find latest news at https://www.otoroshi.io/blog
       ...
       |- a basic workflow to create a resource is to get a template of the resource first using the GET /apis/:group/v1/:resource_plural_name/_template endpoint, modify it according to user needs then create the new resource instance using the create or upsert endpoint
  ...
  def systemPrompt(user: Option[BackOfficeUser], currentUrl: Option[String], now: java.time.ZonedDateTime, env: Env): String = {
    ...
    val contextBlock =
      s"""
         ...
         |${env.allResources.resources.map(r => s"  - ${r.kind}: { group: ${r.group}, resource_plural_name: ${r.pluralName} }").mkString("\n")}

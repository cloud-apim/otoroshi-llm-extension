...
class FaithfulnessGuardrail extends Guardrail {
  ...
  def createStatements(userInput: String, ref: String, model: Option[String], attrs: TypedMap)(using env: Env, ec: ExecutionContext): Future[Seq[String]] = {
    ...
    env.adminExtensions.extension[AiExtension].flatMap(_.states.provider(ref).flatMap(_.withModel(model).getChatClient())) match {
  ...
  def createVerdicts(context: String, statements: Seq[String], outOfScope: Boolean, ref: String, model: Option[String], attrs: TypedMap)(using env: Env, ec: ExecutionContext): Future[Seq[FaithfulnessOutput]] = {
  ...
  override def pass(_messages: Seq[ChatMessage], config: JsObject, provider: Option[AiProvider], chatClient: Option[ChatClient], attrs: TypedMap)(using ec: ExecutionContext, env: Env): Future[GuardrailResult] = {
    ...
    val model = config.select("model").asOptString.map(_.trim).filter(_.nonEmpty)
    ...
    createStatements(userInput, provider, model, attrs).flatMap { statements =>
      createVerdicts(context, statements, outOfScope, provider, model, attrs).flatMap { verdicts =>

    ...
    env.adminExtensions.extension[AiExtension].flatMap(_.states.provider(config.ref)) match {
      ...
      case Some(provider) => provider.getChatClient() match {
        case Some(client) => {
          ...
          client.call(ChatPrompt(config.preChatMessages.map(pm => pm.copy(content = applyEl(pm.content))) ++ Seq(ChatMessage("user", prompt)) ++ config.postChatMessages.map(pm => pm.copy(content = applyEl(pm.content)))), ctx.attrs).map {

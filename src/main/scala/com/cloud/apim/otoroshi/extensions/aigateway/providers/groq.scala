...
class GroqApi(baseUrl: String = GroqApi.baseUrl, token: String, timeout: FiniteDuration = 3.minutes, env: Env) extends ApiClient[GroqApiResponse, OpenAiChatResponseChunk] {
  ...
  override def callWithToolSupport(method: String, path: String, body: Option[JsValue], mcpConnectors: Seq[String], attrs: TypedMap, nameToFunction: Map[String, String], maxCalls: Int, currentCallCounter: Int, acc: UsageAccumulator)(using ec: ExecutionContext): Future[Either[JsValue, GroqApiResponse]] = {
    ...
    if (body.flatMap(_.select("tools").asOpt[JsArray]).exists(_.value.nonEmpty)) {
      call(method, path, body, acc).map(_.map(_.toOpenAi)).flatMap {
...
case class GroqChatClientOptions(
    ...
    // opt-in native /responses endpoint (`POST /openai/v1/responses`, beta at groq)
...
class GroqChatClient(api: GroqApi, options: GroqChatClientOptions, id: String) extends ChatClient with NativeResponsesSupport {
  ...
  override protected def responsesProviderKind: String = "Groq"
  ...
  // documented as unsupported by groq: the endpoint 400s on them instead of ignoring them
  override protected def responsesUnsupportedParams: Seq[String] = Seq("previous_response_id", "store", "truncation", "include", "safety_identifier", "prompt_cache_key", "prompt")
  ...
  override def call(prompt: ChatPrompt, attrs: TypedMap, originalBody: JsValue)(using ec: ExecutionContext, env: Env): Future[Either[JsValue, ChatResponse]] = {
    ...
    val callF = if (!hasToolsInRequest && api.supportsTools && (options.wasmTools.nonEmpty || options.mcpConnectors.nonEmpty || options.a2aConnectors.nonEmpty)) {
      ...
      api.callWithToolSupport("POST", "/openai/v1/chat/completions", Some(mergedOptions ++ tools ++ Json.obj("messages" -> prompt.jsonWithFlavor(ChatMessageContentFlavor.OpenAi))), options.mcpConnectors, attrs, nameToFunction, options.maxFunctionCalls, 0, acc)
    } else {
      api.call("POST", "/openai/v1/chat/completions", Some(mergedOptions ++ Json.obj("messages" -> prompt.jsonWithFlavor(ChatMessageContentFlavor.OpenAi))), acc)
    }
    callF.map {
  ...
  override def stream(prompt: ChatPrompt, attrs: TypedMap, originalBody: JsValue)(using ec: ExecutionContext, env: Env): Future[Either[JsValue, Source[ChatResponseChunk, ?]]] = {
    ...
    val callF = if (!hasToolsInRequest && api.supportsTools && (options.wasmTools.nonEmpty || options.mcpConnectors.nonEmpty || options.a2aConnectors.nonEmpty)) {
      ...
      val nameToFunction = LlmFunctions.nameToFunction(options.wasmToolsNoInline)
      api.streamWithToolSupport("POST", "/openai/v1/chat/completions", Some(mergedOptions ++ tools ++ Json.obj("messages" -> prompt.jsonWithFlavor(ChatMessageContentFlavor.OpenAi))), options.mcpConnectors, attrs, nameToFunction, options.maxFunctionCalls, 0, acc)
    } else {
      api.stream("POST", "/openai/v1/chat/completions", Some(mergedOptions ++ Json.obj("messages" -> prompt.jsonWithFlavor(ChatMessageContentFlavor.OpenAi))), acc)
    }
    callF.map {
      case Left(err) => err.left

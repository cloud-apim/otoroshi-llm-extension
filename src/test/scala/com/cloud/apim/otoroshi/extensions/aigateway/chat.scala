...
class ChatSuite extends munit.FunSuite {

  /*def testOpenai(): Unit = {
    val openaitoken = sys.env("OPENAI_TOKEN")
    ...
    val route = client.forEntity("proxy.otoroshi.io", "v1", "routes").create(Json.parse(
      s"""{
         |  "id": "${routeId}",
         |  "name": "openai",
         |  "frontend": {
         |    "domains": [
         |      "openai.oto.tools/chat"
         |    ]
         |  },
         |  "backend": {
         |    "targets": [
         ...
         |  ]
         |}""".stripMargin)).awaitf(10.seconds)(otoroshi.executionContext)
    assert(route.created, "openai route has not been created")
    assert(provider.created, "openai provider has not been created")
    Utils.await(2.seconds)
    val resp = client.call("POST", s"http://openai.oto.tools:${port}/chat", Map.empty, Some(Json.parse(s"""{
         |  "messages": [
         |    {
         |      "role": "user",
         ...
         |  ]
         |}""".stripMargin))).awaitf(10.seconds)(otoroshi.executionContext)
    assertEquals(resp.status, 200, "chat route did not respond with 200")
    val pointer = resp.json.at("choices.0.message.content")
    assert(pointer.isDefined, "no message content")
    assert(pointer.get.asString.nonEmpty, "no message")
    println(s"openai message: ${pointer.asString}")
    otoroshi.stop()
  }*/
